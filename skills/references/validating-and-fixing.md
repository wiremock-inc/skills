...
2. For each stub do the following:
   ...
   2. Make a test request for the stub mapping with `curl` via `Bash`, against the mock API's base
      URL. There is no MCP tool for making arbitrary HTTP requests, so this must be a real HTTP
      call.
