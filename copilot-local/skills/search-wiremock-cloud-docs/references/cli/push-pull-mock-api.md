...
```shell theme={null}
wiremock mock-apis pull <mock-api-id> <mock-api-id> <mock-api-id>
```
...
If you have modified your Mock APIs in WireMock Cloud, you can re-pull them by running the `mock-apis pull` command
...
Or you can run the `mock-apis pull` command specifying the `<service-id>` from your local WireMock environment file:
...
```shell theme={null}
wiremock mock-apis pull payment-mock-api
```

As with pulling multiple Mock APIs, you can re-pull multiple services by appending multiple `<service-id>`:

```shell theme={null}
wiremock mock-apis pull payment-mock-api order-mock-api
```

If you want to re-pull all the Mock APIs you have defined in your local WireMock environment file, you can run the
`mock-apis pull` command without any mock API ID or service ID and specifying the `--all` flag:

```shell theme={null}
wiremock mock-apis pull --all
```

The `--into=<service-id>` flag can be used to pull data from the supplied mock API into the service entry in the
WireMock environment file matching the `--into` value. In this case, only stub mappings and API documents will be
updated; service settings (e.g. name, description) will remain unchanged. This option is valuable in cases where a
fork of your service exists that you want to merge into the existing service.

For example, given an environment file containing:
...
If a forked version of that Mock API exists in WireMock Cloud with the ID `45fgh6k2`, running the following command
will update the stub mappings and API documents in `./payment-mock-api` with the content from the fork, while leaving
the service settings unchanged:

```shell theme={null}
wiremock mock-apis pull 45fgh6k2 --into=payment-mock-api
```

After the command, the environment file remains unchanged:
...
The WireMock CLI `mock-apis pull` command supports pulling mock APIs from an environment via the use of the `--profile` parameter.
...
```shell theme={null}
wiremock mock-apis push <service-id>
```

You can also push multiple Mock APIs at once by appending multiple `<service-id>`:

```shell theme={null}
wiremock mock-apis push payment-mock-api order-mock-api
```
...
As with the `mock-apis pull` command, you can pass a `--wiremock-dir` argument to override the default `.wiremock` directory.

If the `cloud_id` is missing from the local mock api definition, the `mock-apis push` command will try to create a new Mock API
in WireMock Cloud before pushing the local Mock API.

If you want to push all the Mock APIs defined in your local WireMock environment file, you can run the `mock-apis push`
